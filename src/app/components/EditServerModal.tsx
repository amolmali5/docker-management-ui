...
export default function EditServerModal({ server, onClose, onServerUpdated }: EditServerModalProps) {
  ...
  const [protocol, setProtocol] = useState(server.protocol);
  const [useTLS, setUseTLS] = useState(!!server.cert);
  const [ca, setCa] = useState(server.ca || '');
  const [cert, setCert] = useState(server.cert || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/api/servers/${server.id}`, serverData);
      onServerUpdated();
      onClose();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center">
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 md:mx-auto">
        <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">Edit Docker Server</h3>
          <button
            onClick={onClose}
